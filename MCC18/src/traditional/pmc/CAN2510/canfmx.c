			              ...
			              unsigned long *filters )
{
    ...
    if( bufferNum == 0 )
    {
        CAN2510SetSingleMaskXtd( 0, mask );
	CAN2510SetSingleFilterXtd( 0, filters[0] );
	CAN2510SetSingleFilterXtd( 1, filters[1] );
    }
    else
    {
        CAN2510SetSingleMaskXtd( 1, mask );
	for( currFilter = 0; currFilter < 4; currFilter++ )
	    CAN2510SetSingleFilterXtd( currFilter + 2, filters[currFilter] );
    }
