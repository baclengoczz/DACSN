...
#else
		   !strcasecmp(longName, opt->longName))
#endif
		/*@=nullpass@*/
	{
	    break;
                   ...
                   toupper (shortName) == toupper (opt->shortName)) {
#endif
	    break;
	}
    }
...
#ifdef MICROCHIP
            /* Check for "-s=arg" option.  If equal sign is immediately
             * following -s, then make sure to skip past equal sign when
             * setting up con->os->nextCharArg.  If = sign is somewhere in
             * the string, set longArg to NULL so that it is not the if
             * statement that is entered when expanding the option's
             * argument. */
            if (*origOptString == '=')
              {
                origOptString++;
